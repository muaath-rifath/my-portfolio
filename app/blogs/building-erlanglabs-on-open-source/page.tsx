...
export default function ErlanglabsArticlePage() {
  return <article className="mx-auto w-full min-w-0 max-w-3xl px-6 pb-24 pt-28 sm:px-8 sm:pt-36">
    ...
    <nav aria-label="Article contents" className="mb-12 border-b border-border pb-8"><h2 className="mb-3 text-sm font-semibold">In this article</h2><ol className="space-y-1 text-sm text-muted-foreground dark:text-foreground/90">{headings.map((heading, index) => <li key={heading} className="flex min-w-0 items-start gap-2"><span aria-hidden="true" className="shrink-0 py-3 tabular-nums">{index + 1}.</span><a href={"#" + sectionId(heading)} className="min-w-0 rounded-sm py-3 underline decoration-border underline-offset-4 hover:decoration-current focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-ring">{heading}</a></li>)}</ol></nav>
